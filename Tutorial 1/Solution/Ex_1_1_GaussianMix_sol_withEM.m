...
% 
% 
% mu = [mu1; mu2; mu1 + 2; mu2 + 2];
% sigma2 = [sigma12; sigma22; sigma12+0.5;sigma12; sigma22; sigma12+0.5];
% w = [1/3; 1/3];
% theta = [mu; sigma2; w];
% % change sample for that ! but we compute for the one we generated to simplify things 
% loglikVal = getGaussianMixLogLikelihood(GaussMix_sample, theta);
...
funToMinimize = @(theta) -getGaussianMixLogLikelihood(GaussMix_sample, theta);
options = optimoptions(@fmincon,'Display','iter');
Aeq = [0 0 0 0 1 1];
beq = 1;
theta0 = [0 0 1 1 0.5 0.5];
lb = [-Inf -Inf 0 0 0 0];
...
while (norm(mu - mu_current, 2) > TolX)
    ...
    for j = 1:k
        ...
        pdf_values(:, j) = normpdf(GaussMix_sample, mu(j), sigma(j));
    end
    ...
    for j = 1:k
        ...
        % points
        temp = W(:, j)' * (GaussMix_sample - mu(j)).^2;
        variance = temp./sum(W(:, j), 1);
...
% see file Ex_1_2_GaussianMix2d_sol
