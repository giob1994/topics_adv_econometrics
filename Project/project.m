...
PCA_percentages  = [50, 75, 85, 95];

PN = length(PCA_percentages)*length(NN_architecture);
...
[pca_coeff, ~, ~, ~, explained, ~] = pca(X_mnist);
explained = cumsum(explained);
PCA_sizes = ones(size(PCA_percentages));
for j = 1:length(PCA_percentages)
    PCA_sizes(j) = find(explained > PCA_percentages(j), 1);
end
...
if (FFT_2D_FLAG == 1)
    ...
    [pca_coeff, ~, ~, ~, explained, ~] = pca(X_notMnist_ft2);
else 
    [pca_coeff, ~, ~, ~, explained, ~] = pca(X_notMnist);
end
explained = cumsum(explained);
PCA_sizes = ones(size(PCA_percentages));
for j = 1:length(PCA_percentages)
...
FFT_2D_FLAG = 1; % IF = 1, use the 2D Fourier Transform on the data;
...
if (FFT_2D_FLAG == 1)
    ...
    [pca_coeff, ~, ~, ~, explained, ~] = pca(X_fashion_ft2);
else 
    [pca_coeff, ~, ~, ~, explained, ~] = pca(X_fashion);
end
explained = cumsum(explained);
PCA_sizes = ones(size(PCA_percentages));
for j = 1:length(PCA_percentages)
...
for PC_dim = PCA_sizes
    ...
    if (FFT_2D_FLAG == 0)
        % PCA of the Fashion data:
        X_fashion_pca = X_fashion * pca_coeff;
        X_fashion_test_pca = X_fashion_test * pca(X_fashion_test);
    else
        % PCA for the 2D FFT data:
        X_fashion_pca = X_fashion_ft2 * pca_coeff;
